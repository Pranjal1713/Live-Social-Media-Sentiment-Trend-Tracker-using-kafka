...
# if __name__ == '__main__':
#     # --- FIX: Corrected file path to go up one directory ---
#     with open('./data/mock_social_media.json', 'r') as f:
#         mock_data = json.load(f)

#     while True:
#         # Get a random message
#         random_message = random.choice(mock_data)
        
#         # Add a timestamp
#         random_message['timestamp'] = time.strftime("%Y-%m-%d %H:%M:%S")
        
#         print(f"Sending message: {random_message}")
        
#         # Send it to our 'social-media-posts' topic
#         producer.send('social-media-posts', random_message)
