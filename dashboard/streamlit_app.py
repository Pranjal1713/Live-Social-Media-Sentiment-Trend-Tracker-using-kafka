...
# # social-media-sentiment-tracker/dashboard/streamlit_app.py
# import streamlit as st
# import pandas as pd
# import time
# import numpy as np
...
# st.set_page_config(
#     page_title="Live Social Media Sentiment Tracker",
#     layout="wide",
# )

# st.title("Live Social Media Sentiment & Trend Tracker 📊")

# # --- Data Initialization ---
# # We'll use a session state to store our data so it persists across reruns
# if 'df_posts' not in st.session_state:
#     st.session_state.df_posts = pd.DataFrame(columns=[
#         'timestamp', 'text', 'user', 'platform', 'user_followers', 
#         'likes', 'retweets', 'city', 'country', 'sentiment_score', 'hashtags'
#     ])

# # --- Placeholders for Dynamic Content ---
# st.header("Live Metrics")
# col1, col2, col3 = st.columns(3)
# total_posts_placeholder = col1.empty()
# avg_likes_placeholder = col2.empty()
# avg_retweets_placeholder = col3.empty()

# st.header("Visualizations")
# col4, col5 = st.columns(2)
# sentiment_placeholder = col4.empty()
# hashtags_placeholder = col5.empty()

# col6, col7 = st.columns(2)
# platform_placeholder = col6.empty()
# country_placeholder = col7.empty()

# st.header("Latest Posts")
# latest_posts_placeholder = st.empty()


# # --- Main Loop to Simulate and Update Data ---
# while True:
#     # Simulate receiving new data (replace this with actual data connection later)
#     new_data = {
#         'timestamp': [pd.to_datetime('now', utc=True)],
#         'text': [np.random.choice([
#             "Loving the new features! #awesome", "This is the worst update ever. #fail",
#             "Incredible movie! #cinema", "Political debate was insightful. #politics",
#             "The economy is not looking good. #recession", "What a beautiful day! #happy"
#         ])],
#         'user': [f'user_{np.random.randint(100, 999)}'],
#         'platform': [np.random.choice(['Twitter', 'Reddit', 'Instagram'])],
#         'user_followers': [np.random.randint(100, 100000)],
#         'likes': [np.random.randint(0, 5000)],
#         'retweets': [np.random.randint(0, 2000)],
#         'city': [np.random.choice(['New York', 'London', 'Tokyo', 'Paris', 'Sydney'])],
#         'country': [np.random.choice(['USA', 'UK', 'Japan', 'France', 'Australia'])],
#         'sentiment_score': [np.random.uniform(-1, 1)],
#         'hashtags': [[f'#topic{np.random.randint(1,10)}' for _ in range(np.random.randint(1, 3))]]
#     }
#     new_df = pd.DataFrame(new_data)

#     # Append new data and keep the last 1000 posts
#     st.session_state.df_posts = pd.concat([st.session_state.df_posts, new_df], ignore_index=True)
#     st.session_state.df_posts = st.session_state.df_posts.tail(1000)

#     # --- Update Metrics ---
#     total_posts = len(st.session_state.df_posts)
#     avg_likes = st.session_state.df_posts['likes'].mean()
#     avg_retweets = st.session_state.df_posts['retweets'].mean()

#     with total_posts_placeholder.container():
#         st.metric(label="Total Posts", value=f"{total_posts:,}")
#     with avg_likes_placeholder.container():
#         st.metric(label="Average Likes", value=f"{avg_likes:,.0f}")
#     with avg_retweets_placeholder.container():
#         st.metric(label="Average Retweets", value=f"{avg_retweets:,.0f}")

#     # --- Update Charts ---
#     # Sentiment Over Time
#     with sentiment_placeholder.container():
#         if not st.session_state.df_posts.empty:
#             sentiment_over_time = st.session_state.df_posts.set_index('timestamp')['sentiment_score'].rolling('10s').mean()
#             fig_sentiment = px.line(sentiment_over_time, title='Sentiment Score Over Time (10s Rolling Average)')
#             st.plotly_chart(fig_sentiment, use_container_width=True)

#     # Trending Hashtags
#     with hashtags_placeholder.container():
#         if not st.session_state.df_posts.empty:
#             all_hashtags = st.session_state.df_posts.explode('hashtags')
#             top_hashtags = all_hashtags['hashtags'].value_counts().nlargest(10).reset_index()
#             top_hashtags.columns = ['hashtag', 'count']
#             fig_hashtags = px.bar(top_hashtags, x='hashtag', y='count', title='Top 10 Trending Hashtags')
#             st.plotly_chart(fig_hashtags, use_container_width=True)

#     # Posts by Platform
#     with platform_placeholder.container():
#         if not st.session_state.df_posts.empty:
#             platform_counts = st.session_state.df_posts['platform'].value_counts().reset_index()
#             platform_counts.columns = ['platform', 'count']
#             fig_platform = px.pie(platform_counts, names='platform', values='count', title='Posts by Platform')
#             st.plotly_chart(fig_platform, use_container_width=True)

#     # Posts by Country
#     with country_placeholder.container():
#         if not st.session_state.df_posts.empty:
#             country_counts = st.session_state.df_posts['country'].value_counts().reset_index()
#             country_counts.columns = ['country', 'count']
#             fig_country = px.bar(country_counts, x='country', y='count', title='Posts by Country')
#             st.plotly_chart(fig_country, use_container_width=True)
            
#     # Latest Posts Table
#     with latest_posts_placeholder.container():
#         st.dataframe(st.session_state.df_posts[['timestamp', 'platform', 'user', 'text', 'likes', 'retweets', 'country']].tail(10))
